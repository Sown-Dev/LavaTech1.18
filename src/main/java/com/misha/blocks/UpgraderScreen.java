...
public class UpgraderScreen extends AbstractContainerScreen<UpgraderContainer> {
    private ResourceLocation GUI = new ResourceLocation(LavaPlus.MODID, "textures/gui/upgrader_gui.png");
    ...
    public UpgraderScreen(UpgraderContainer container, Inventory inv, Component name) {
    ...
    protected void renderLabels(PoseStack matrixStack, int mouseX, int mouseY) {
        ...
        if(mouseX>relX+11 && mouseX< relX+28 && mouseY>relY+13 && mouseY<relY+65){
            ...
            drawString(matrixStack, Minecraft.getInstance().font,   "uses " + UpgraderBE.baseUsage*menu.blockEntity.cactive+"FE/tick", mouseX-relX+9, mouseY-relY+8, 0xffffff);
    ...
    protected void renderBg(PoseStack matrixStack, float partialTicks, int mouseX, int mouseY) {
        ...
        if(menu.blockEntity.ccounter>0 && menu.blockEntity.cactive>0){
            ...
            int bwidth=(int)(30*((double) menu.blockEntity.ccounter/((double)UpgraderBE.baseTime/(double)menu.blockEntity.cactive)));

            this.blit(matrixStack, relX + 99, relY + 22, 0, 0, bwidth, 17);

        }
        int barX = relX+10;
        int barY = relY+65-(int)(((double)menu.getEnergy()/(double) UpgraderBE.capacity)*52);
        RenderSystem.setShaderTexture(0, BAR);
        this.blit(matrixStack, barX,barY, 0, 0, 17, (int)(((double) menu.getEnergy()/ (double)UpgraderBE.capacity)*52));
